...
newrow = m+img_size
newcol = n+img_size
newimg  = np.zeros([newrow,newcol,d], dtype=np.uint8)
for b in range(0,d):
    start = int(img_size/2)
    newimg[start:start+m,start:start+n,b] = img2[:,:,b]
...
sampledir = os.path.join(data_root,'data','img2_rgb_size'+str(img_size),'sample')
mk_file(sampledir)
clear_dir(sampledir)
...
for label in range(1,len(class_dict)+1):
    ...
    classdir = os.path.join(data_root,'data','img2_rgb_size'+str(img_size),'sample','class'+str(label))
    mk_file(classdir)
